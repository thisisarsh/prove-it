...
export function PhoneInputCluster() {
        ...
        if (phone.length < e.target.value.length) {
            switch (e.target.value.length) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    e.target.value = e.target.value.substring(0,3) + '(' + e.target.value.substring(3);
                    break;
                case 7:
                    e.target.value = e.target.value + ') ';
                    break;
                case 8:
                    ...
                    break;
                case 9:
                    e.target.value = e.target.value.substring(0,8) + ' ' + e.target.value.substring(8);
                    break;
                case 12:
                    e.target.value = e.target.value + '-'
                    break;
                case 13:
                    e.target.value = e.target.value.substring(0,12) + '-' + e.target.value.charAt(12);
                    break;
            }
        }
    ...
    return (
        ...
        <div className='welcome-message'>
            Welcome to HomeTrumpeter, NewUser!<br/>
            Please enter your phone number to finalize your account
        </div>
        <Form className="PhoneInputCluster" onSubmit={handleSubmit}>
        <Form.Control type="phone" placeholder="phone" id='phone-input' onChange={e => handlePhoneChange(e)}/>
            {/*Buttons*/}
